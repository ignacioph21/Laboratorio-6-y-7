...
def fcd_analysis(i_ref, i_def, roi, hstar, roi2=None):
    x, y, w, h = roi
    ...
    window1d = np.abs(tukey(roi[-1], 0.005))
...
for i in range(1, 8):
    ...
    height_field = fcd_analysis(i_ref, i_def, roi, hstar)
    # plot_height_field(height_field, i_teo=i_teo, roi=roi, PXtoM=carriers[0].PXtoM)

    roi2 = selectSquareROI("i_def: seleccionar region de interes", (normalize_image(height_field) * 255).astype(np.uint8), scale_kwargs=scale_roi_kwargs)
    cv2.destroyWindow("i_def: seleccionar region de interes")
    height_field = fcd_analysis(i_ref, i_def, roi, hstar, roi2=roi2)
    

    h_effs.append(hp*1000)
    max_heights.append(np.max(height_field)*1000)

plt.scatter(h_effs, max_heights)
plt.xlabel("Altura efectiva [mm]")
plt.ylabel("Altura de la gota [mm]")
