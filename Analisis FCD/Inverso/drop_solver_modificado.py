...
def drop_shape(R0, ca, s=None, volume=None):
    R0 = R0[0] if type(R0)==np.ndarray else R0 # TODO: esto lo agregué yo poruqe causaba fallos. No pude rastrear porqué a veces es array y aveces no.
    """
    ...
    y = odeint(yl, np.array([0.0, 0.0, 0.0]), np.array(s), args=(1/R0,)) 
...
volume = 5 # \muL.
contact_angle = 150 # Degrees. 
main(volume=1e-9*volume, ca=contact_angle)
