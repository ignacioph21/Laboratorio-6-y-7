...
y = np.loadtxt('output50.csv', delimiter=',', skiprows=1) # TODO: ya hay algo que se llama y más adelante, cuidado.

# Definir la resolución del height field
resolution = 256
size = (resolution, resolution)

gain = 10 * 1000   # TODO: Multiplicar h y esto es lo mismo. ¿Supongo que es algo que tenemos que calibrar.?
frequency = 25
...
imageio.imwrite('test image reference.png', (reference_checkerboard * 255).astype(np.uint8))
imageio.imwrite('test image displaced.png', (displaced_checkerboard * 255).astype(np.uint8))
imageio.imwrite('test image height field.png', (normalize_image(height_field) * 255).astype(np.uint8))
